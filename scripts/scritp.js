    ...
    words.forEach(word => {
        ...
        <div class="card bg-white px-5 py-10 shadow-sm text-center space-y-6">
            ...
            <div class="flex justify-between mt-6">
                ...
                <button onclick="pronounceWord('${word.word}')" class="btn btn-soft btn-info text-[#374957]"><i class="fa-solid fa-volume-high"></i></button>
            </div>
        </div>
        `
...
document.getElementById('btn-search').addEventListener('click', () => {
    ...
    fetch('https://openapi.programming-hero.com/api/words/all')
        .then(res => res.json())
        .then(words => {
            const allData = words.data
            // console.log(allData)
            const filterWord = allData.filter((word) => word.word.toLowerCase().includes(searchValue))
